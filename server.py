...
genai.configure(api_key="AIzaSyCUpnaKTGFDjsUzN2ijHobqK-0X0cy_DoY")
...
model = genai.GenerativeModel(
    ...
    system_instruction="follow the chat history and the converation should be done so taht you should impersonate the history no change of thngs and always try to ask question that inetiate  conversation"
)
...
async def chat_with_enotes(websocket):
    try:
        await websocket.send("WebSocket connection established with Enotes!")  # Send welcome message
